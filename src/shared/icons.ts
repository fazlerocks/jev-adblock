/**
 * Inline Lucide icons (https://lucide.dev, ISC licence). Inlined so the extension ships no CDN
 * references and stays CSP-clean. Each entry is the inner markup of a 24x24 stroke icon.
 */
const PATHS = {
...
export function icon(name: IconName, size = 16, className = ""): string {
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide ${className}" aria-hidden="true">${PATHS[name]}</svg>`;
}

/** Replace every `<i data-icon="name">` placeholder in the document with the inline SVG. */
